    ...
    public static void runArchon() {
        ...
        while(true){
            try{
                ...
                builtgardner=tryToBuild(RobotType.GARDENER,RobotType.GARDENER.bulletCost);
                if ( builtgardner==true)
                    gardnertotal++;
                if ( gardnertotal>20)
                    rc.disintegrate();
    ...
    public static void runGardener(){
        while(true){
            try{
                ...
                if(rc.getTeamBullets()>GameConstants.VICTORY_POINTS_TO_WIN)
                    rc.donate(10000);
                Clock.yield();
            }catch(Exception e){
                e.printStackTrace();
